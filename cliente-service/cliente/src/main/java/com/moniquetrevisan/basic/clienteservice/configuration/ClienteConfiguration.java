package com.moniquetrevisan.basic.clienteservice.configuration;
...
public class ClienteConfiguration {
	...
	public Binding bindingCadastro(TopicExchange topic, Queue cadastroQueue) {
		return BindingBuilder.bind(cadastroQueue).to(topic).with(routingKeyCadastro);
	}

	@Bean
	public Binding bindingAtualizacao(TopicExchange topic, Queue atualizacaoQueue) {
		return BindingBuilder.bind(atualizacaoQueue).to(topic).with(routingKeyAtualizacao);
	}
